...
class BibtexReader(Reader):
    ...
    def __filter_people(people: str) -> List[Tuple(str, str)]:
        ...
        return [BibtexReader.__filter_name(x) for x in people if not x.startswith('other')]
    ...
    def __custom_filter(entry: Dict[str, Union[str, int]]) -> Dict[str, Union[str, int, List, Set]]:
        ...
        for key in entry:
            item = BibtexReader.__filter_common(entry[key])
            if key in BibtexReader.__filter_dict:
                item = BibtexReader.__filter_dict[key](item)
            entry[key] = item
        return entry

    __parser = bibtexparser.bparser.BibTexParser()
    __parser.customization = __custom_filter
    ...
    def __call__(self):
        return bibtexparser.load(self.fp, parser=self.__parser)
